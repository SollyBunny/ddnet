...
class CWarList : public CComponent
{
	...
	*/
	...
	/*
		...
		pair<PlayerName, FilePath>
	*/
	std::vector<std::pair<std::string, std::string>> m_vSuslist;
	...
	*/
	...
	void GetSuslistPathByName(const char *pName, int Size, char *pPath);
	...
	*/
	...
	bool AddSus(const char *pFolder, const char *pName);
...
public:
	...
	void AddSimpleSus(const char *pName);
	void RemoveSimpleSus(const char *pName);
	...
	bool IsSuslist(const char *pName);
