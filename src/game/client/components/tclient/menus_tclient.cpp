...
void CMenus::RenderSettingsTClientConfigs(CUIRect MainView)
{
	...
	struct SIntStage { int m_Value; };
	struct SStrStage { std::string m_Value; };
	struct SColStage { unsigned m_Value; };
	...
	struct SIntState { CLineInputNumber m_Input; int m_LastValue = 0; bool m_Inited = false; };
	struct SStrState { CLineInputBuffered<512> m_Input; bool m_Inited = false; };
	struct SColState { unsigned m_LastValue = 0; unsigned m_Working = 0; bool m_Inited = false; };
		...
		if (pVar->m_Type == SConfigVariable::VAR_INT)
		{
			...
			else
			{
				...
				if (Ui()->DoEditBox(&State.m_Input, &InputBox, EditBoxFontSize))
				{
					...
					{
						if (NewVal < pInt->m_Min) InRange = false;
						if (pInt->m_Max != 0 && NewVal > pInt->m_Max) InRange = false;
					}
